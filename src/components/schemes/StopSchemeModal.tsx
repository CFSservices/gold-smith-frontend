...
interface StopSchemeModalProps {
  scheme: Scheme;
  visible: boolean;
  onHide: () => void;
  onConfirmStop: (data: {
...
export function StopSchemeModal({
  scheme,
  visible,
  onHide,
  onConfirmStop,
}: StopSchemeModalProps) {
  ...
  // Handle stop scheme
  const handleStopScheme = () => {
    if (!otpVerified || !comments.trim()) return;
    
    setError(null);
    setLoading((prev) => ({ ...prev, stopScheme: true }));
    ...
    try {
      ...
      setError('Failed to stop scheme. Please try again.');
    } finally {
      setLoading((prev) => ({ ...prev, stopScheme: false }));
    }
  };
    ...
    >
      <div className="space-y-6">
        ...
        <div className="text-base text-secondary-900 dark:text-white">
          Are you sure you want to stop this scheme? By Stopping, you will not be able to reverse this action. Customer will also not be able to make payments against this scheme further.
        </div>
        ...
        <div>
          <label className="block text-sm font-medium text-secondary-900 dark:text-white mb-2">
            Reason for stopping scheme<span className="text-red-500">*</span>
          </label>
          <InputTextarea
            value={comments}
        ...
        {otpVerified && (
          <div className="pt-4 border-t border-gray-200 dark:border-secondary-700">
            <Button
              ...
              icon="pi pi-square"
              severity="danger"
              onClick={handleStopScheme}
              className="w-full"
              loading={loading.stopScheme}
              disabled={!comments.trim() || loading.stopScheme}
            />
          </div>
        )}
      </div>
